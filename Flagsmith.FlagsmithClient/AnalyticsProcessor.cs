    ...
    public class AnalyticsProcessor : IAnalyticsProcessor
    {
        ...
        public async Task Flush()
        {
            ...
            try
            {
                ...
                _Logger?.Log("Analytics posted: " + analyticsJson);
                AnalyticsData.Clear();
                _Logger?.Log("Analytics cleared: " + analyticsJson);
            }
            catch (HttpRequestException ex)
            {
                _Logger?.LogError("Flagsmith", "Analytics api error: " + ex.Message);
            }
            catch (TaskCanceledException)
            {
                _Logger?.LogWarning("Flagsmith", "Analytics request cancelled: Api request takes too long to respond");
            }
            _LastFlushed = DateTime.Now;
        }
        /// <summary>
